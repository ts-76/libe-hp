...
Claude が `npm run setup` を実行して、libe-hp の npm パッケージ、Git hook、安全チェックを準備します。

PC 全体に入れる mise、Node.js、Git、GitHub CLI は、このプロジェクトではなく `global-tools-setup` で準備します。
...
まだ準備できていない場合は、先にワークスペース直下の `global-tools-setup` を実行してください。

libe-hp のプロジェクトを準備する場合:

```sh
npm run setup
```

`npm run setup` は、libe-hp のディレクトリ内だけを準備します。

- npm パッケージをインストールします
- Git hook をインストールします
- プロジェクト固有の安全チェックを実行します
- Git hook のチェックを一度実行します
...
| コマンド | 内容 |
| :-- | :-- |
| `npm run setup` | libe-hp の npm パッケージ、Git hook、安全チェックを準備する |
